 ...
 */
function detectIOCType(ioc: string): 'hash' | 'ip' | 'domain' | 'url' | 'email' | 'unknown' {
  // MD5: 32 hex chars
 ...
 */
async function searchFilesystemForHashes(
  hashes: string[],
  searchPath: string,
  matches: IOCMatch[]
): Promise<void> {
 ...
 */
async function searchProcesses(iocs: string[], iocTypes: Map<string, string>): Promise<IOCMatch[]> {
      ...
      for (const ioc of iocs) {
        ...
        if (searchFields.includes(iocLower)) {
          ...
          const match: IOCMatch = {
            id: crypto.randomUUID(),
            ioc,
            iocType: (iocType as IOCMatch['iocType']) || 'domain',
 ...
 * Start IOC hunt
 */
export async function startIOCHunt(config: IOCHuntConfig): Promise<IOCMatch[]> {
  try {
    ...
    const iocTypes = new Map<string, string>();
    const hashIOCs: string[] = [];
